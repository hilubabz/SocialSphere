...
import { Bell, Heart, MessageCircle, UserPlus, Share, Settings, X } from "lucide-react"
...
const NotificationsPage = () => {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "share":
        return <Share className="w-5 h-5 text-emerald-600" />
      default:
        ...
        <div className="max-w-4xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-gradient-to-r from-emerald-500/20 to-emerald-600/20 rounded-2xl border border-emerald-500/20">
                <Bell className="w-6 h-6 text-emerald-400" />
              </div>
              <div>
                <h1 className="text-white text-2xl font-bold tracking-tight mb-1">Notifications</h1>
            ...
            <div className="flex items-center gap-3">
              {unreadCount > 0 && (
                <button
                  ...
                  className="text-emerald-400 text-sm font-medium hover:text-emerald-300 transition-colors duration-200"
                >
                  Mark all as read
                </button>
              )}
            ...
            {[
              ...
              >
                ...
                {tab.key === "unread" && unreadCount > 0 && (
                  <span className="ml-2 bg-emerald-500 text-white text-xs px-2 py-0.5 rounded-full">{unreadCount}</span>
                )}
              </button>
            ))}
          </div>
            ...
            {filteredNotifications.map((notification) => (
              <div
                ...
                className={`group relative bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl p-5 hover:from-slate-700/60 hover:to-slate-800/60 transition-all duration-300 border border-slate-600/20 hover:border-slate-500/30 ${
                  !notification.isRead
                    ? "border-emerald-500/30 bg-gradient-to-br from-emerald-900/20 to-slate-900/60"
                    : ""
                }`}
